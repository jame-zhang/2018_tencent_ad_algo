...
class DeepFM(BaseEstimator, TransformerMixin):
    def __init__(self, field_sizes,
                 ...
                 deep_layers=[128, 128], dropout_deep=[1.0, 1.0, 1.0],
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():
            ...
            self.op = tf.contrib.linalg.LinearOperatorLowerTriangular(tf.transpose(self.ones,[0,3,1,2])) # None *k * F *F
    ...
    def _initialize_weights(self):
        ...
        weights["static_lr_embeddings"] = tf.Variable(
            tf.random_normal([self.total_feature_sizes[0], 1], 0.0, 0.0001),
            name="static_lr_embeddings")
        weights["dynamic_lr_embeddings"] = tf.Variable(
            tf.random_normal([self.total_feature_sizes[1], 1], 0.0, 0.0001),
        ...
        weights["layer_0"] = tf.Variable(
            np.random.uniform(-glorot, glorot, size=(input_size, self.deep_layers[0])), dtype=np.float32)
        weights["bias_0"] = tf.Variable(np.random.uniform(-glorot, glorot, size=(1, self.deep_layers[0])),
                                                        dtype=np.float32)  # 1 * layers[0]
        for i in range(1, num_layer):
            glorot = np.sqrt(6.0 / (self.deep_layers[i-1] + self.deep_layers[i]))
            weights["layer_%d" % i] = tf.Variable(
                np.random.uniform(-glorot, glorot,  size=(self.deep_layers[i-1], self.deep_layers[i])),
    ...
    def get_batch(self, static_index, dynamic_index, dynamic_lengths, y, batch_size, index):
        ...
        padded_index = dynamic_index[start:end]
        batch_ids = []
        for i in range(len(padded_index)):
            buf_ids = []
            for j in padded_index[i]:
                if len(j) >= self.dynamic_max_len:
                    buf_ids = buf_ids + j[:self.dynamic_max_len]
                else:
                    buf_ids = buf_ids + self.pad_seq(j, self.dynamic_max_len)
            batch_ids.append(buf_ids)
        return static_index[start:end], np.array(batch_ids), dynamic_lengths[start:end],\
    ...
    def fit(self, train_static_index, train_dynamic_index, train_dynamic_lengths, train_y,
        ...
        """
        print("fit begin")
        train_dynamic_index = train_dynamic_index
        valid_dynamic_index = valid_dynamic_index
    ...
    def pad_seq(self, seq, max_len):
        seq = seq + [0 for i in range((max_len - len(seq)))]
        return seq