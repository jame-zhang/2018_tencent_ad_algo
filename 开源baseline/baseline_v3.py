...
def batch_predict(data,index):
    ...
    feature_after_onehot = dict()  #储存每个特征onehot之后的维度
    df_after_onehot = pd.DataFrame() #将onehot之后的数据保存为df
    for feature in one_hot_feature:
        enc.fit(data[feature].values.reshape(-1, 1))
        train_a=enc.transform(train[feature].values.reshape(-1, 1))
        feature_after_onehot[feature] = train_a.shape[0]
        temp_df_after_onehot = pd.DataFrame(train_a.todense())
        temp_df_after_onehot.columns = [feature + str(i) for i in range(temp_df_after_onehot.shape[1])]
        df_after_onehot = pd.concat([df_after_onehot,temp_df_after_onehot], axis=1)
...
def LGB_predict(train_x,train_y,test_x,res,index):
    ...
    clf = lgb.LGBMClassifier(
        boosting_type='gbdt', num_leaves=63, reg_alpha=0.0, reg_lambda=0.01,
        max_depth=-1, n_estimators=100, objective='binary',
        subsample=0.8, colsample_bytree=0.8, subsample_freq=1,
        learning_rate=0.05, min_child_weight=[0.001,0.1,0.01], random_state=2018, n_jobs=-1  #,scale_pos_weight=20
    )
